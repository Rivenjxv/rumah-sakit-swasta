...
export default function AgamaPage() {
  ...
  const [editForm, setEditForm] = useState({ kodeAgama: '', namaAgama: '' })
  ...
  const fetchData = () => {
    ...
    const url = `http://160.20.104.98/api/Agama?page=${page}&perPage=${perPage}`
  ...
  const handleEdit = (item) => {
    setEditItem(item)
    setEditForm({ kodeAgama: item.kodeAgama, namaAgama: item.namaAgama })
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`http://160.20.104.98/api/Agama/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`
      }
  ...
  const handleUpdate = async () => {
    ...
    if (!editForm.kodeAgama.trim() || !editForm.namaAgama.trim()) {
      setErrorEdit('Kode dan Nama Agama wajib diisi.')
      return
    }

    if (
      editItem.kodeAgama === editForm.kodeAgama &&
      editItem.namaAgama === editForm.namaAgama
    ) {
    ...
    try {
      const res = await fetch(`http://160.20.104.98/api/Agama/${editItem.agamaId}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (!res.ok) {
        if (res.status === 409) {
          const isSameCode = data.some(
            (item) =>
              item.kodeAgama === editForm.kodeAgama &&
              item.agamaId !== editItem.agamaId
          )
          if (isSameCode) {
            throw new Error('Kode Agama sudah digunakan oleh entri lain.')
          }
        }
    ...
    <div className="d-flex">
      ...
      <div className="flex-grow-1" style={{ marginLeft: sidebarWidth, transition: 'margin 0.25s ease' }}>
        <div className="container py-4">
          <h2 className="mb-4 text-primary fw-bold">
            <i className="bi bi-journal-text me-2" /> Daftar Agama
          </h2>
          ...
          <div className="mb-3 d-flex justify-content-between align-items-center">
            ...
            >
              {showForm ? 'Tutup Form' : '➕ Tambah Agama'}
            </Button>
          </div>
          ...
          {showForm && (
            <div className="mb-4">
              <div className="p-4 border rounded bg-light shadow-sm">
                <FormAgama token={localStorage.getItem("token")}
                  onSuccess={() => {
                    setShowForm(false)
                    setPage(1)
                    setReloadFlag(r => !r)
                      ...
                      <tr key={item.agamaId || index}>
                        ...
                        <td className="text-center">
                          ...
                          <Button
                            ...
                            onClick={() => handleDelete(item.agamaId)}
                          >
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Edit Agama</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Control type="hidden" value={editForm.kodeAgama} readOnly />
            <Form.Group className="mb-3">
              <Form.Label>Nama Agama</Form.Label>
              <Form.Control
                type="text"
                value={editForm.namaAgama}
                onChange={(e) => setEditForm({ ...editForm, namaAgama: e.target.value })}
              />
            </Form.Group>
